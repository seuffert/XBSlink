...
namespace XBSlink
{
    ...
    class xbs_sniffer
    {
        ...
        // ORIGINAL CODE DON't WORKS (I CAN SEE THE GAMES) :S USING  0.9.5.3 CODE
        ...
        public xbs_sniffer(ICaptureDevice dev, bool use_special_mac_filter, List<PhysicalAddress> filter_special_macs, bool only_forward_special_macs, xbs_node_list node_list, xbs_nat NAT, GatewayIPAddressInformationCollection gateways, bool exclude_gateway_ips)
        {
            ...
            this.pdev_filter_only_forward_special_macs = only_forward_special_macs;
            this.pdev_filter_exclude_gatway_ips = exclude_gateway_ips;
            this.create_gateway_filter(gateways);
            this.injected_macs_hash.Capacity = 40;
            this.sniffed_macs_hash.Capacity = 10;
            this.sniffed_macs.Capacity = 10;
            this.node_list = node_list;
            if (!(dev is LibPcapLiveDevice))
            {
                throw new ArgumentException("pcap caputure device is not a LibPcapLiveDevice");
            }
            this.pdev = (LibPcapLiveDevice)dev;
            this.pdev.OnPacketArrival += new PacketArrivalEventHandler(xbs_sniffer.OnPacketArrival);
            this.pdev.Open(DeviceMode.Promiscuous, this.readTimeoutMilliseconds);
            this.setPdevFilter();
            if ((Environment.OSVersion.Platform == PlatformID.Win32NT) && (this.pdev is WinPcapDevice))
            {
                ((WinPcapDevice)this.pdev).MinToCopy = 10;
            }
            xbs_messages.addInfoMessage(" - sniffer created on device " + this.pdev.Description, xbs_message_sender.SNIFFER);
            this.dispatcher_thread = new Thread(new ThreadStart(this.dispatcher));
            this.dispatcher_thread.IsBackground = true;
            this.dispatcher_thread.Priority = ThreadPriority.AboveNormal;
            this.dispatcher_thread.Start();
        }
...
//        }
        ...
        public void dispatch_packet(ref RawCapture rawPacket)
        {
            ...
            PhysicalAddress srcMAC = new PhysicalAddress(src_mac);
            bool is_injected_packet = false;
            lock (this.injected_macs_hash)
            {
                is_injected_packet = this.injected_macs_hash.Contains(srcMAC.GetHashCode());
            }
            if (!is_injected_packet)
            {
                if (this.NAT.NAT_enabled)
                {
                    this.NAT.deNAT_outgoing_packet_PacketDotNet(ref packet_data, dstMAC, srcMAC);
                }
                xbs_sniffer_statistics.packet_count++;
                this.node_list.distributeDataPacket(dstMAC, packet_data);
                int srcMac_hash = srcMAC.GetHashCode();
                bool pdevfilter_needs_change = false;
                lock (this.sniffed_macs_hash)
                {
                    if (!this.sniffed_macs_hash.Contains(srcMac_hash))
                    {
                        this.sniffed_macs_hash.Add(srcMac_hash);
                        lock (this.sniffed_macs)
                        {
                            this.sniffed_macs.Add(srcMAC);
                        }
                        pdevfilter_needs_change = true;
                    }
                }
...
// ORIGINAL CODE DON't WORKS (I CAN SEE THE GAMES) :S
//
...
//        }
        ...
        public void injectRemotePacket(ref byte[] data, PhysicalAddress dstMAC, PhysicalAddress srcMAC)
        {
            ...
            lock (this.injected_macs_hash)
            {
                if (!this.injected_macs_hash.Contains(srcMac_hash))
                {
                    this.injected_macs_hash.Add(srcMac_hash);
                    this.addMacToKnownMacListFromRemoteNodes(srcMAC);
                }
            }
            try
            {
            ...
            if (p_arp != null)
            {
                foreach (IPAddress ip in this.gateway_ips)
                {
                    if (p_arp.TargetProtocolAddress.Equals(ip) || p_arp.SenderProtocolAddress.Equals(ip))
                    {
                        return;
                    }
                }
            ...
            if (p_ipv4 != null)
            {
                if (this.pdev_filter_exclude_gatway_ips)
                {
                    foreach (IPAddress ip in this.gateway_ips)
                    {
                        if (p_ipv4.DestinationAddress.Equals(ip) || p_ipv4.SourceAddress.Equals(ip))
                        {
                            return;
                        }
                    }
            ...
            if ((((p_udp == null) || !this.pdev_filter_wellknown_ports) || (p_udp.DestinationPort <= 0x3ff)) && (((p_tcp == null) || !this.pdev_filter_wellknown_ports) || (p_tcp.DestinationPort <= 0x3ff)))
            {
                if (this.NAT.NAT_enabled)
                {
                    this.NAT.NAT_incoming_packet_PacketDotNet(ref data, dstMAC, srcMAC, ref p, ref p_ipv4, ref p_arp);
                }
                try
                {
                    this.pdev.SendPacket(data, data.Length);
                }
                catch (PcapException pex)
                {
                    xbs_messages.addInfoMessage(string.Concat(new object[] { "!! error while injecting packet from ", srcMAC, " to ", dstMAC, " (", data.Length, ") : ", pex.Message }), xbs_message_sender.SNIFFER, xbs_message_type.FATAL_ERROR);
                }
                catch (ArgumentException aex)
                {
                    xbs_messages.addInfoMessage(string.Concat(new object[] { "!! error while injecting packet from ", srcMAC, " to ", dstMAC, " (", data.Length, ") : ", aex.Message }), xbs_message_sender.SNIFFER, xbs_message_type.FATAL_ERROR);
                }
            }
        }
...
//        }
        ...
        public void setPdevFilter()
        {
            List<string> exclude_filter_list = new List<string>();
            List<string> include_filter_list = new List<string>();
            if (PhysicalAddressToString(this.pdev.MacAddress).Length > 0)
            {
                exclude_filter_list.Add("ether host " + PhysicalAddressToString(this.pdev.MacAddress));
            }
            if (this.pdev_filter_exclude_gatway_ips && (this.pdev_filter_gateways.Length > 0))
            {
                exclude_filter_list.Add(this.pdev_filter_gateways);
            }
            if (this.pdev_filter_wellknown_ports)
            {
                exclude_filter_list.Add("ip and (dst portrange 1-1023 or src portrange 1-1023)");
            }
            if (!this.pdev_filter_use_special_macs || !this.pdev_filter_only_forward_special_macs)
            {
                include_filter_list.Add("(udp and ((ip host 0.0.0.1) or (dst portrange 3074-3075) or (dst portrange 14000-14001))) ");
                if (this.pdev_filter_forward_high_port_broadcasts)
                {
                    include_filter_list.Add("ether dst ff:ff:ff:ff:ff:ff and udp and dst portrange 1024-65535");
                }
            }
            lock (this.pdev_filter_known_macs_from_remote_nodes)
            {
                if (this.pdev_filter_known_macs_from_remote_nodes.Count > 0)
                {
                    if (!this.pdev_filter_use_special_macs || !this.pdev_filter_only_forward_special_macs)
                    {
                        include_filter_list.Add("ether dst " + string.Join(" or ether dst ", this.pdev_filter_known_macs_from_remote_nodes.ConvertAll<string>(pa => PhysicalAddressToString(pa)).ToArray()));
                    }
                    exclude_filter_list.Add("ether src " + string.Join(" or ether src ", this.pdev_filter_known_macs_from_remote_nodes.ConvertAll<string>(pa => PhysicalAddressToString(pa)).ToArray()));
                }
            }
            if (this.pdev_filter_use_special_macs)
            {
                lock (this.pdev_filter_special_macs)
                {
                    if (this.pdev_filter_special_macs.Count > 0)
                    {
                        include_filter_list.Add("ether src " + string.Join(" or ether src ", this.pdev_filter_special_macs.ConvertAll<string>(pa => PhysicalAddressToString(pa)).ToArray()));
                    }
                }
            }
            lock (this.sniffed_macs)
            {
                if ((this.sniffed_macs.Count > 0) && (!this.pdev_filter_use_special_macs || !this.pdev_filter_only_forward_special_macs))
                {
                    include_filter_list.Add("ether src " + string.Join(" or ether src ", this.sniffed_macs.ConvertAll<string>(pa => PhysicalAddressToString(pa)).ToArray()));
                }
            }
            string exlude_filter_string = "( " + string.Join(" ) or (", exclude_filter_list.ToArray()) + " ) ";
            string include_filter_string = "( " + string.Join(" ) or (", include_filter_list.ToArray()) + " ) ";
            string f = (exclude_filter_list.Count > 0) ? this.pdev_filter_template : this.pdev_filter_template_only_include;
            f = f.Replace("{include_filters}", include_filter_string);
            if (exclude_filter_list.Count > 0)
            {
                f = f.Replace("{exlude_filters}", exlude_filter_string);
            }
            try
            {
                this.pdev.Filter = f;
            ...
            {
                xbs_messages.addInfoMessage("!! - ERROR setting pdev filter, using fallback - please inform developer. pdev_filter: " + f, xbs_message_sender.SNIFFER, xbs_message_type.FATAL_ERROR);
                this.pdev.Filter = this.pdev_filter_fallback;
            }
        }
