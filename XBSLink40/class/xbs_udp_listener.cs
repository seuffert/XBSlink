...
namespace XBSlink
{
    ...
    class xbs_udp_listener
    {
        ...
        public delegate void PMReceivedHandler(xbs_node_message_msgpm udp_msg, bool IsSended);
        public static event PMReceivedHandler PMReceived;

        public delegate void ChatReceivedHandler(string UserName, string Message);
        public static event ChatReceivedHandler ChatReceived;
...
# endif
                ...
                case xbs_node_message_type.CHATMSG:
                    if (sending_node != null)
                    {
                        ...
                        string message = msg_chat.getChatMessage();

                        if (ChatReceived != null)
                            ChatReceived(sending_node.nickname, message);

                        xbs_chat.addChatMessage(sending_node.nickname, message);
                    }
                    break;
